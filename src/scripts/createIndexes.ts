...
async function createIndexes() {
  try {
    ...
    try {
      await mongoose.connection.db!.collection('productcategories').createIndex(
        {
          name: 'text',
          description: 'text'
        },
    ...
    try {
      await mongoose.connection.db!.collection('searchhistories').createIndex(
    ...
    for (const { collection, index, name } of performanceIndexes) {
      try {
        await mongoose.connection.db!.collection(collection).createIndex(index as any, { name });
