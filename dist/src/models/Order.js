...
// SCHEMA DEFINITIONS
// =============================================================================
const OrderLineItemSchema = new mongoose_1.Schema({
    productId: {
...
const MobileStatusHistorySchema = new mongoose_1.Schema({
    status: {
        type: String,
...
OrderSchema.virtual('canBeReturned').get(function () {
    if (this.mobileStatus.current !== 'delivered' || !this.deliveredAt) {
        return false;
    }
 ...
 */
OrderSchema.methods.updateMobileStatus = async function (status, note, location) {
    ...
    this.mobileStatus.current = status;
    ...
    if (statusMessages[status]) {
        await this.sendNotification('push', 'Order Update', statusMessages[status]);
    }
};
/**
 * Send notification to customer
 */
OrderSchema.methods.sendNotification = async function (type, title, message) {
    // Check if user has enabled this notification type
    const preferences = this.notificationPreferences;
    const isEnabled = (type === 'push' && preferences.pushEnabled) ||
    ...
    console.log(`Notification sent to user ${this.user}: ${title} - ${message}`);
};
/**
...
OrderSchema.pre('save', async function (next) {
    ...
    if (this.isNew && this.mobileStatus.history.length === 0) {
        this.mobileStatus.history = [{
                status: 'placed',
                timestamp: this.placedAt || new Date()
            }];
    }
    next();
});
