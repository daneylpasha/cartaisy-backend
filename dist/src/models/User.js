...
const MarketingConsentSchema = new mongoose_1.Schema({
    state: {
        type: String,
...
UserSchema.virtual('hasShippingAddress').get(function () {
    return this.addresses.some(addr => addr.type === 'shipping');
});
UserSchema.virtual('hasBillingAddress').get(function () {
 ...
 */
UserSchema.methods.updateAddress = function (index, updates) {
 ...
 */
UserSchema.methods.removeAddress = function (index) {
    if (index < 0 || index >= this.addresses.length) {
        return false;
    }
 ...
 * Set default address
 */
UserSchema.methods.setDefaultAddress = function (index) {
    if (index < 0 || index >= this.addresses.length) {
        return false;
    }
