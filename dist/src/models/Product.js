...
// SCHEMA DEFINITIONS
// =============================================================================
const VariantInventorySchema = new mongoose_1.Schema({
...
const VariantOptionsSchema = new mongoose_1.Schema({
    option1: { type: String },
    option2: { type: String },
    option3: { type: String }
}, { _id: false });
...
const ProductReviewsSchema = new mongoose_1.Schema({
 ...
 */
ProductSchema.methods.updateAnalytics = async function () {
    ...
    this.analytics.engagementScore = Math.round((this.analytics.viewCount * 0.1) +
...
ProductSchema.methods.isAvailable = function () {
    ...
    return this.variants.some(variant => variant.inventory.quantity > 0 &&
...
ProductSchema.pre('save', function (next) {
    ...
    this.inventoryTracking.totalQuantity = this.variants.reduce((total, variant) => total + (variant.inventory?.quantity || 0), 0);
 ...
 */
ProductSchema.statics.findByText = function (query, options = {}) {
 ...
 */
ProductSchema.statics.findLowStock = function (threshold) {
    const query = {
        status: 'active',
