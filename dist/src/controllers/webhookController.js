...
exports.handleInventoryUpdate = exports.handleCustomerUpdate = exports.handleCustomerCreate = exports.handleOrderPaid = exports.handleOrderUpdate = exports.handleOrderCreate = exports.handleProductDelete = exports.handleProductCreate = exports.handleProductUpdate = exports.verifyWebhookSignature = void 0;
...
const verifyWebhookSignature = (req, res, next) => {
    try {
        ...
        const secret = tenant_1.tenantConfig.shopify.webhookSecret;
        if (!signature) {
            return res.status(401).json({
                error: 'Missing webhook signature'
            });
        }
        const isValid = (0, exports.verifyWebhookSignature)(body, signature, secret);
        if (!isValid) {
...
const handleProductUpdate = async (req, res) => {
    try {
        ...
        if (existingProduct) {
            ...
            const mergedData = await (0, syncService_1.syncProductData)(shopifyProduct, existingProduct);
            // Handle any data conflicts
            const { resolved } = await (0, syncService_2.handleDataConflicts)(existingProduct.toObject(), mergedData, 'product');
            // Update the product
            ...
            // Create new product
            await (0, shopifyService_1.syncProduct)(shopifyProduct.id.toString(), shopifyProduct);
...
const handleProductCreate = async (req, res) => {
    try {
        ...
        if (existingProduct) {
            ...
            return (0, exports.handleProductUpdate)(req, res);
        }
        // Create new product
        await (0, shopifyService_1.syncProduct)(shopifyProduct.id.toString(), shopifyProduct);
...
exports.handleProductCreate = handleProductCreate;
/**
...
const handleOrderCreate = async (req, res) => {
    try {
        ...
        // Find or create user
        let user = await User_1.default.findOne({ email: shopifyOrder.email });
        if (!user) {
        ...
        await newOrder.sendNotification('email', 'Order Confirmation', `Your order #${shopifyOrder.order_number} has been received and is being processed.`);
...
exports.handleOrderCreate = handleOrderCreate;
/**
...
const handleOrderUpdate = async (req, res) => {
    try {
        ...
        if (existingOrder.mobileStatus.current !== newMobileStatus) {
            await existingOrder.updateMobileStatus(newMobileStatus, 'Status updated from Shopify', shopifyOrder.shipping_address?.city);
        }
...
exports.handleOrderUpdate = handleOrderUpdate;
/**
 * Handle order payment webhook from Shopify
 */
const handleOrderPaid = async (req, res) => {
    try {
        const shopifyOrder = req.body;
        ...
        await existingOrder.updateMobileStatus('processing', 'Payment confirmed', undefined);
        // Send payment confirmation notification
        await existingOrder.sendNotification('push', 'Payment Confirmed', `Payment for order #${shopifyOrder.order_number} has been confirmed. We're processing your order now!`);
...
const handleCustomerCreate = async (req, res) => {
    try {
        ...
        const existingUser = await User_1.default.findOne({ email: shopifyCustomer.email });
        if (existingUser) {
            // Update existing user with Shopify data
            const mergedData = await (0, syncService_1.syncCustomerData)(shopifyCustomer, existingUser);
            ...
            // Create new user
            const userData = await (0, syncService_1.syncCustomerData)(shopifyCustomer);
...
exports.handleCustomerCreate = handleCustomerCreate;
/**
...
const handleCustomerUpdate = async (req, res) => {
    try {
        ...
        if (existingUser) {
            ...
            const mergedData = await (0, syncService_1.syncCustomerData)(shopifyCustomer, existingUser);
            const { resolved } = await (0, syncService_2.handleDataConflicts)(existingUser.toObject(), mergedData, 'customer');
...
exports.handleCustomerUpdate = handleCustomerUpdate;
/**
...
exports.handleInventoryUpdate = handleInventoryUpdate;
// Helper functions (imported from shopifyService)
const syncService_2 = require("../services/syncService");
