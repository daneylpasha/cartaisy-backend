...
exports.getOrderAnalytics = exports.createSupportTicket = exports.rateOrder = exports.getOrderTracking = exports.requestReturn = exports.cancelOrder = exports.updateOrderStatus = exports.createOrder = exports.getOrder = exports.getUserOrders = void 0;
const mongoose_1 = __importDefault(require("mongoose"));
const Order_1 = __importDefault(require("../models/Order"));
const OrderTracking_1 = __importDefault(require("../models/OrderTracking"));
...
const getUserOrders = async (req, res) => {
    try {
        ...
        if (startDate || endDate) {
            ...
            if (endDate)
                filter.placedAt.$lte = new Date(endDate);
        }
...
const getOrder = async (req, res) => {
    try {
        ...
        if (order.shipping.trackingNumber) {
            tracking = await OrderTracking_1.default.findOne({
...
const createOrder = async (req, res) => {
    try {
        const userId = req.user?._id;
        const { lineItems, billingAddress, shippingAddress, shipping, specialInstructions, deliveryPreferences, source = 'mobile', channel = 'app', campaignId } = req.body;
        if (!userId) {
            res.status(401).json({
                success: false,
                message: 'Authentication required'
        ...
        await order.sendNotification('email', 'Order Confirmation', `Your order #${order.orderNumber} has been received and is being processed.`);
        res.status(201).json({
            success: true,
            data: {
                order: {
...
const getOrderTracking = async (req, res) => {
    try {
        const { orderId } = req.params;
        const userId = req.user?._id;
        const order = await Order_1.default.findById(orderId).select('user shipping mobileStatus');
        if (!order) {
            res.status(404).json({
                success: false,
                message: 'Order not found'
...
const rateOrder = async (req, res) => {
    try {
        const { orderId } = req.params;
        const { overallRating, deliveryRating, packagingRating, productQualityRating, customerServiceRating, comment } = req.body;
        const userId = req.user?._id;
...
const getOrderAnalytics = async (req, res) => {
    try {
        ...
        const statusDistribution = await Order_1.default.aggregate([
            {
                $match: {
                    user: new mongoose_1.default.Types.ObjectId(userId)
                }
        ...
        const spendingTrend = await Order_1.default.aggregate([
            {
                $match: {
                    user: new mongoose_1.default.Types.ObjectId(userId),
                    placedAt: { $gte: startDate }
