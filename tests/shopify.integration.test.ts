...
describe('Shopify Integration Tests', () => {
  ...
  afterAll(async () => {
    // Clean up test data
    await Product.deleteMany({ handle: { $regex: /test-/ } });
    await Order.deleteMany({ 'customer.email': 'shopify.test@example.com' });
    await User.deleteMany({ email: 'shopify.test@example.com' });
