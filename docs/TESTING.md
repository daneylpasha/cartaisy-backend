...
Lint command: `npm run lint` runs `eslint .` against ESLint 9 flat config in `eslint.config.mjs`. Added 2026-07-29, replacing a `.eslintrc.js` that could never have worked (see below). **It is enforced in GitHub Actions.** The `Type Check & Package Checks` job runs `npm run lint` between `type-check` and the security audit, with no `continue-on-error` — a lint failure fails the job exactly as a type-check failure does. That placement is deliberate: lint and type-check are both hard gates, while the audit step is explicitly tolerant of findings, so the two strict steps sit together. The suppression baseline is what makes a hard gate safe from day one: the pre-existing errors are recorded (255 when the gate was introduced; 149 since the 2026-07-30 auto-fix burn-down), so the job goes green immediately. Be precise about what that gate then guarantees, because the obvious phrasing overstates it. Suppression entries record **how many** violations a file has for a rule, not which ones. So a violation in a file with no recorded entry for that rule fails, a count above the recorded one fails, and fixing one violation while adding one in a *different* file fails — but fixing one and adding another **in the same file for the same rule** leaves the count unchanged and passes. The gate prevents the debt from growing; it does not reject every individual new violation. `.gitlab-ci.yml` contains a `test:lint` job that runs `npm ci`, then `npm run lint`, then `npm run format:check` on merge requests and on `develop`/`main`. Adding the `lint` script therefore changes that job's behaviour: it previously died on a missing `lint` script and now gets past it. It still cannot pass, because **`format:check` is not defined in `package.json` either** — so the failure has moved one line down rather than gone away. Whether that GitLab pipeline runs at all is not answerable from the repository; the project lives on GitHub. Anyone relying on it should treat `test:lint` as broken until `format:check` exists.
...
- Linting is enforced on the path that gates `main`: the `Type Check & Package Checks` job runs `npm run lint` and fails when the recorded error counts are exceeded. Two things it does *not* do. It does not prove the code is clean — 149 errors are suppressed and 1,596 warnings are not gated at all. And it does not catch a violation swapped for another of the same rule in the same file, because the baseline counts rather than identifies. The gate stops the debt growing; it neither measures it nor freezes it.
- `.gitlab-ci.yml`'s `test:lint` job is broken and stayed broken. It runs `npm run lint` followed by `npm run format:check`; the first now works, the second refers to a script that does not exist in `package.json`. Either define `format:check` (a `prettier --check .` — a `.prettierrc` is already present) or drop that line. Worth deciding first whether the GitLab pipeline is live at all, since the project is hosted on GitHub and the file may simply be vestigial.
- `.yarn/**` is ignored by the lint config. ESLint's default traversal includes `.cjs`, which the ported `**/*.js` ignore does not cover, and the plugin's base config carries no `files` restriction, so the TypeScript parser was being applied to the vendored 2.2 MB `.yarn/releases/yarn-3.5.1.cjs` on every run. No rule currently applies to `.cjs`, so it produced no violations and never entered the suppression baseline — but it dominated the runtime, and it would have begun failing the moment any rule did apply to `.cjs`.
- The lint baseline is not clean. Turning linting on for the first time surfaced **255 errors and 1,596 warnings**. The errors are recorded per file and per rule in `eslint-suppressions.json` (generated once with `eslint --suppress-all`), and the rules keep their real severity, so new violations still fail while the existing debt does not. The 106 auto-fixable errors of the original 255 were burned down on 2026-07-30 with a single `npx eslint . --fix` pass across 37 files — `object-shorthand` 70, `prefer-template` 24, `prefer-const` 11 and `prefer-arrow-callback` 1 all cleared mechanically — after which the baseline was shrunk with `npx eslint . --prune-suppressions`, which can only lower or remove entries, never add them (regenerating with `--suppress-all` would have silently absorbed any *new* violations into the baseline, which is exactly what it must never do). The baseline now records **149 errors**: `@typescript-eslint/no-unused-vars` 92 (mostly `catch (error)` params ESLint 9 newly flags; removing them needs judgment, not a fixer), `@typescript-eslint/no-require-imports` 20, `no-useless-escape` 18 (suggestion-only rule, no autofix — the `validation.ts` regexes were deliberately not hand-edited), `prefer-const` 8 (destructurings where some bindings are reassigned), `no-duplicate-imports` 7 (no fixer exists), `no-empty` 2, `@typescript-eslint/no-empty-object-type` 1, `@typescript-eslint/no-unsafe-function-type` 1. What remains is the judgment-required residue; there is no further one-command win. As on the dashboard, the suppressions file records counts per file per rule, not identities: it stops debt growing, it does not freeze individual violations, and it must never be regenerated with `--suppress-all` to turn a red run green.
