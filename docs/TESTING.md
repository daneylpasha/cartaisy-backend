...
- Linting is enforced on the path that gates `main`: the `Type Check & Package Checks` job runs `npm run lint` and fails when the recorded error counts are exceeded. Two things it does *not* do. It does not prove the code is clean — 255 errors are suppressed and 1,596 warnings are not gated at all. And it does not catch a violation swapped for another of the same rule in the same file, because the baseline counts rather than identifies. The gate stops the debt growing; it neither measures it nor freezes it.
