...
- Linting is enforced on the path that gates `main`: the `Type Check & Package Checks` job runs `npm run lint` and fails on any violation outside the recorded baseline. What it does *not* prove is that the code is clean — 255 errors are suppressed and 1,596 warnings are not gated at all. The gate stops the debt growing; it does not measure the debt.
