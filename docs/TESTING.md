...
- Linting is not enforced on the path that actually gates `main`. No GitHub Actions job runs `npm run lint`, so nothing stops a violation merging. Wiring it into `Type Check & Package Checks` is a deliberate follow-up decision rather than an oversight — turning on a new gate is a bigger call than adding a script.
