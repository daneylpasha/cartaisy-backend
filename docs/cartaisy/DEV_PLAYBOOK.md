...
Phases 0-2 of `docs/cartaisy/ROADMAP.md` have met gates: Phase 0 (2026-07-22), Phase 1 (2026-07-22), and Phase 2 — the first branded Android app build — (2026-08-04), evidence linked in the roadmap. Phase 2 carries open, non-blocking follow-up items (see the roadmap). Phase 3 (runtime branding): all four ticket items are implemented (2026-08-04, PRs #137, #115, #116, #13), but the gate itself is not yet verified — no one has performed and recorded the live dashboard-edit-to-mobile-app run the gate requires, so it stays open per the roadmap's own evidence rule and this playbook's "never mark something done that wasn't verified." See the roadmap for exactly what closes it. Phase 4 (dashboard consolidation) and Phase 5 (per-merchant push, unblocked now that Phase 2's gate is met) are open as parallel tracks per the roadmap's sequencing and are cut by the orchestrator as capacity allows. Phase 6 needs all prior phases and is not yet open.
