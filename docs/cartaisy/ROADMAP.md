...
Remaining Phase 3 work (tracked as open, does not block the gate above): three disclosed coverage gaps. (1) The hardcoded `t("common.companyName")` string (5 UI locations) is not yet wired to dashboard branding. (2) A handful of `getTokenValue()`/`AppImage.resolveTokenColor()` tint call sites read a separate, static token registry and never pick up a runtime color change — not even after a restart, which reruns the same bundle; only a code fix making these reads reactive, or a rebuild with a new bundled default, closes this one. This was previously a code-review finding only; the 2026-08-06 run reproduced it live — on the Account/Sign-in screen, the "Sign In" button and the "Cartaisy" footer wordmark stayed on the old static maroon/red color while the header correctly showed the new `#0000FF`, in the same screen, at the same moment. (3) New, found during the same run: the dashboard (`cartaisy-dashboard`) has two client-side bugs that block anyone pointing it at a non-production backend — `lib/api/generated/authentication/authentication.ts`'s `getLoginUrl()` returns a hardcoded absolute production URL instead of honoring `NEXT_PUBLIC_API_URL` (patched locally for this session; not yet a PR), and `lib/auth/auth-context.tsx`'s `initAuth()`/`refreshUser()` discarded the fresh `storeId`/`storeName` returned by `/auth/profile` in favor of a stale cached copy, causing a session to flash the correct store then revert (patched locally for this session; not yet a PR). Neither blocks the gate above — both were worked around live — but both should get real PRs since they'll trip up the next person running the dashboard against a local backend.
