...
- Runtime branding is implemented end-to-end: `primaryColor`, `secondaryColor`, and `logoUrl` are dashboard-editable and apply on next app launch without a rebuild, with a bundled on-brand fallback for a fresh install — but the Phase 3 gate itself is not yet verified; no live dashboard-edit-to-mobile-app run has ever been performed or recorded (see Phase 3 below). Two further coverage gaps are also disclosed and open: the hardcoded `t("common.companyName")` string (5 UI locations) isn't wired to dashboard branding yet, and a number of `getTokenValue()`/`AppImage` tint call sites read a separate, static token registry and never pick up a runtime color change — not even after a restart, since that just reruns the same bundle; only a code fix or a rebuild closes this (exact call-site count tracked in the `Cartaisy` repo's `docs/STATUS.md`).
...
**All four items implemented 2026-08-04; gate not yet verified — kept open.** Backend `/store/config` serves validated `primaryColor`, `secondaryColor`, and `logoUrl` (PR #137); mobile applies both colors live via Tamagui's `updateTheme` — no rebuild required — with WCAG accessibility contrast guardrails on both fields (PRs #115, #116), and renders the logo across all 7 mobile surfaces with a bundled build-time fallback so a fresh install is on-brand from the first frame (PRs #104–#111); the dashboard's color/logo editor writes through the backend API only, no local schema duplication (PR #13). Full surface-by-surface accounting: `Cartaisy` repo's `docs/MOBILE_BRANDING_SPLIT_MATRIX.md` (added in #116).

Per this roadmap's own rule (gate evidence must be recorded, not assumed) and `docs/cartaisy/DEV_PLAYBOOK.md`'s house rule ("never mark something done that wasn't verified"), the gate itself stays open. Confirmed directly with the operator (2026-08-06): the exact behavior this gate describes — a dashboard branding edit persisting to a live backend, then a mobile app instance fetching and applying it on next launch — has never been performed or recorded. Unlike Phase 1's cart→checkout→webhook trace and Phase 2's physical-device install log, today's evidence is component-level only: backend contract, mobile consumption, and dashboard editor are each independently tested/verified (mobile's hook tests exercise a mocked store, not the real fetch→persist→hydrate→apply chain), but no single run has connected them, and a fresh install's on-brand-from-first-frame claim is also unverified live.

To close the gate: change a color/logo in the dashboard against a real Store, confirm it persists via `/store/config`, then launch (or relaunch) a mobile app instance pointed at that store and observe the new branding on next launch without a rebuild — plus confirm a fresh install shows on-brand (bundled-default) content, not an unbranded blank state. Record the run here, dated, once done.

Remaining Phase 3 work (separate from the gate-closing run above, tracked as open): two disclosed coverage gaps, both recorded in the split matrix — the hardcoded `t("common.companyName")` string (5 UI locations) is not yet wired to dashboard branding, and a handful of `getTokenValue()`/`AppImage.resolveTokenColor()` tint call sites read a separate, static token registry and never pick up a runtime color change — not even after a restart, which reruns the same bundle; only a code fix making these reads reactive, or a rebuild with a new bundled default, closes this one.
